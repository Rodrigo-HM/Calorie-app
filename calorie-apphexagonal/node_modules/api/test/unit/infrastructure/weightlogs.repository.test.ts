...
describe("WeightLogsRepositoryLowdb (integration)", () => {
  ...
  // Helper para crear WeightLog con id incremental
  let seq = 0;
  const makeLog = (over: Partial<WeightLogCreateProps> & { userId: string; dateISO: string }): WeightLog =>
    WeightLog.create({
      id: over.id ?? `wl_${++seq}`,
      userId: over.userId,
  ...
  it("sin rango devuelve todos los logs del usuario", async () => {
    await repo.create(makeLog({ userId: "u1", dateISO: "2025-10-10T00:00:00.000Z", weightKg: 80 }));
    await repo.create(makeLog({ userId: "u1", dateISO: "2025-10-20T12:00:00.000Z", weightKg: 79 }));
    await repo.create(makeLog({ userId: "u2", dateISO: "2025-10-20T12:00:00.000Z", weightKg: 82 }));
  ...
  it("from YYYY-MM-DD → T00:00:00.000Z inclusivo", async () => {
    await repo.create(makeLog({ userId: "u1", dateISO: "2025-10-19T23:59:59.999Z", weightKg: 80 }));
    await repo.create(makeLog({ userId: "u1", dateISO: "2025-10-20T00:00:00.000Z", weightKg: 79 }));
    await repo.create(makeLog({ userId: "u1", dateISO: "2025-10-20T12:00:00.000Z", weightKg: 78 }));

    const out = await repo.listByUser("u1", { from: "2025-10-20" });
    const iso = out.map((x) => x.dateISO);
    expect(iso).toEqual(["2025-10-20T00:00:00.000Z", "2025-10-20T12:00:00.000Z"]);
  });

  it("to YYYY-MM-DD → T23:59:59.999Z inclusivo", async () => {
    await repo.create(makeLog({ userId: "u1", dateISO: "2025-10-20T00:00:00.000Z", weightKg: 80 }));
    await repo.create(makeLog({ userId: "u1", dateISO: "2025-10-20T23:59:59.999Z", weightKg: 79 }));
    await repo.create(makeLog({ userId: "u1", dateISO: "2025-10-21T00:00:00.000Z", weightKg: 78 }));
  ...
  it("combina from y to (ambos inclusivos)", async () => {
    await repo.create(makeLog({ userId: "u1", dateISO: "2025-10-10T00:00:00.000Z", weightKg: 80 }));
    await repo.create(makeLog({ userId: "u1", dateISO: "2025-10-20T12:00:00.000Z", weightKg: 79 }));
    await repo.create(makeLog({ userId: "u1", dateISO: "2025-10-30T23:59:59.999Z", weightKg: 78 }));
    await repo.create(makeLog({ userId: "u1", dateISO: "2025-11-01T00:00:00.000Z", weightKg: 77 }));
