...
// Reasigna weightLogs a un único usuario si solo hay 1 usuario en DB
export async function migrateWeightLogsUserToSingleUser(): Promise<void> {
  db.read();
  const users = (db.data?.users as any[] | undefined) ?? [];
  if (users.length !== 1) return;

  const soleUserId = users[0].id;
  const logs = (db.data?.weightLogs as any[] | undefined) ?? [];

  let changed = 0;
  for (const w of logs) {
    if (!w.userId || w.userId === "u1") {
      w.userId = soleUserId;
      changed++;
    }
  }
  if (changed > 0) db.write();
}
